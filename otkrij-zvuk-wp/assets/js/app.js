...
async function loadMusicboardReviews(container) {
    ...
    try {
        ...
        const dataUrl = (typeof themeData !== 'undefined') ? themeData.dataUrl : './data/';
        const response = await fetch(dataUrl + 'reviews.json?v=' + Math.random());
...
async function loadGenres(container) {
    try {
        ...
        genres.forEach(genre => {
            ...
            const siteUrl = (typeof themeData !== 'undefined') ? themeData.siteUrl : '';
            card.href = `${siteUrl}/genre?id=${genre.slug}`;
...
async function loadGenreDetails(container) {
    ...
    if (!genreId) {
        const siteUrl = (typeof themeData !== 'undefined') ? themeData.siteUrl : 'index.html';
        container.innerHTML = `<div class="error-state" style="margin-top:200px;">Nije izabran ni jedan žanr. <a href="${siteUrl}#zanrovi">Vrati se nazad</a></div>`;
        return;
    }
    ...
    try {
        ...
        let html = `
            ...
            <div class="container pb-6" style="padding-bottom: 6rem;">
                <a href="${siteUrl}#zanrovi" style="color: var(--text-secondary); margin-bottom: 2rem; display: inline-block;">&larr; Nazad na sve žanrove</a>
